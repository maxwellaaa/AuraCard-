...
async function resolveFontEmbedCSS(node: HTMLElement): Promise<string> {
  // Google Fonts / CORS 在 Electron（尤其 macOS）上常使 html-to-image 永久挂起；
  // 限时嵌入，失败则跳过字体嵌入，改用本机已加载字体。
  try {
...
async function renderCardPng(
  ...
  try {
    ...
    // 二次兜底：彻底跳过字体嵌入，避免跨域字体/图片拖死 Promise
    if (fontEmbedCSS !== "") {
      return await withTimeout(
        toPng(node, { ...baseOptions, fontEmbedCSS: "" }),
        CARD_CAPTURE_TIMEOUT_MS,
...
async function saveDataUrlAsPng(
  ...
  if (desktop?.writeFile && reuseDirFrom) {
    ...
    const written = await desktop.writeFile({ filePath, dataBase64 });
    if (!written.ok) {
  ...
  return saveBase64File(dataBase64, filename);
}
...
export async function downloadPng() {
  ...
  downloadProgress.value = { current: 0, total: nodes.length };
  selectSticker(null);
  await nextTick();
  await yieldToUi();
  ...
  try {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      const dataUrl = await renderCardPng(nodes[i], preset, fontEmbedCSS);
...
export async function downloadAllCardsZip() {
  ...
  try {
    ...
    const result = await saveBase64File(arrayBufferToBase64(buffer), zipName);
    if (result.canceled) {
      notifyWarning("已取消保存。");
      return;
    }
